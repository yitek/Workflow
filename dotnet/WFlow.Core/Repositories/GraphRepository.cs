...
namespace WFlow.Repositories
{
    public class GraphRepository : IGraphRepository
    {
        ...
        public INode Fetch(string fullname, string version = null)
        {
            var versionCache = this.caches.GetOrAdd(version, (version) => new ConcurrentDictionary<string, Node>());
            return versionCache.GetOrAdd(fullname, (fname) => this.Load(fullname, version) as Node);
        }

        public INode Load(string fullname, string version)
        {
